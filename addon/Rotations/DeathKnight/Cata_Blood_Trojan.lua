...
local items = {
	...
	{ type = "title", text = ">   Blood DK Tank by |c0000CED1Trojan|r   <" },
	{ type = "separator" },
...
local function OnLoad()
	...
	ni.combatlog.registerhandler("Cata_Blood_Trojan", CombatEventCatcher);
	ni.GUI.AddFrame("Cata_Blood_Trojan", items);
end;
-- Unload GUI / Wipe Cache -- 
local function OnUnLoad()
	ni.listener:remove("update_cache", update_cache);
	ni.combatlog.unregisterhandler("Cata_Blood_Trojan");
	ni.GUI.DestroyFrame("Cata_Blood_Trojan");
end;
-- Rotation Priorities --
local queue = {
	"Cache",
...
-----------------------------------
	...
	ni.bootstrap.profile("Cata_Blood_Trojan", queue, abilities, OnLoad, OnUnLoad);
    ...
    ni.bootstrap.profile("Cata_Blood_Trojan", queue, abilities);
end;