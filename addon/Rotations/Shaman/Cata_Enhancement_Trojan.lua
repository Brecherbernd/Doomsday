...
local items = {
	...
	{ type = "title", text = ">   Enchantment Shaman by |c0000CED1Trojan|r   <" },
	{ type = "separator" },
...
local function OnLoad()
	...
	ni.combatlog.registerhandler("Cata_Enhancement_Trojan", CombatEventCatcher);
	ni.GUI.AddFrame("Cata_Enhancement_Trojan", items);
end;
-- Unload GUI / Wipe Cache -- 
local function OnUnLoad()
	ni.listener:remove("update_cache", update_cache);
	ni.combatlog.unregisterhandler("Cata_Enhancement_Trojan");
	ni.GUI.DestroyFrame("Cata_Enhancement_Trojan");
end;
-- Rotation Priorities --
local AoEQueu = {
	"Cache",
...
local function queue()
	...
	ni.bootstrap.profile("Cata_Enhancement_Trojan", queue, abilities, OnLoad, OnUnLoad);
    ...
    ni.bootstrap.profile("Cata_Enhancement_Trojan", queue, abilities);
end;